...
def draw_tank_table(
    ...
    for index, row in enumerate(soup.find_all("tr")):
        ...
        for cell_index, cell in enumerate(columns):
            ...
            # lines = simpleSplit(text, "Helvetica", 9, cell_width)
            # Custom split for header row (if index == 0)
            if index == 0 and '\n' in text:
                lines = text.split('\n')
            else:
                lines = simpleSplit(text, "Helvetica", 9, cell_width)
...
def generate_pdf_lifting(request):
    ...
    if request.method == "POST":
        ...
        p.drawString(40, title_y_position, "LIFTING ANALYSIS") 
        ...
        p.setFont("Helvetica-Bold", 20)  
        p.setFillColor(HexColor("#000000"))  
        p.drawString(40, title_y_position, "LIFTING ANALYSIS REPORT")
        ...
        sections = [
            ...
            ("     3.5   Boom & Lift Data ", report_data),
            ("     3.6   SWL Graph ", report_data),
            ("     3.7   SWL Table ", report_data),
            ("     3.8   Longitudinal Strength ", report_data),
            ("     3.9   Crane Parameters ", report_data),
            ("     3.10   Drilling Platform ", report_data),
        ]
        ...
        abbreviations = {
            "MNET": "Net Moment",
            "MGROSS": "Gross Moment",
            "MRIGGING": "Rigging Moment",
            "WCF": "Wind Correction Factor",
            "WCFRIGGING": "Wind Correction Factor for Rigging",
            "DAF": "Dynamic Amplification Factor",
            "DAFINCL": "DAF including inclination effects",
            "RA": "Actual Radius",
            "MBLOCK": "Moment from the Block",
            "MHOOK": "Moment from the Hook",
            "SHLBE": "Safe Hook Load at Boom End",
            "SHLUB": "Safe Hook Load at Upper Boom",
            "RM": "Maximum Radius",
            "SWLCORR": "Corrected Safe Working Load",
            "SWLREQ": "Required Safe Working Load",
            "UCL": "Load Utilization Ratio",
            "UCR": "Utilization Crane Ratio",
        ...
        raw_result_data = request.POST.get("result_data")
        if raw_result_data :
            try :
                result_data = json.loads(raw_result_data)
                html_result = result_html_from_json(result_data)
        ...
        y_position -= 10
        caption_text = "Table 3-5 Boom & Lift Data"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        y_position -= 10
        caption_text = "Figure 3-1 SWL Graph"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        y_position -= 10
        caption_text = "Table 3-6 SWL Table"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        draw_text(p, "LONGITUDINAL STRENGTH", left_margin, y_position - 30)
        y_position -= 10
        caption_text = "Table 3-7 Longitudinal Strength"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        raw_strength_table = request.POST.get("strength_table")
        if raw_strength_table :
            try :
                strength_data = json.loads(raw_strength_table)
                strength_data_html = strength_html_from_json(strength_data)

                soup = BeautifulSoup(strength_data_html, "html.parser")
                y_position = draw_table_with_colors(
                    soup,
                    y_position - 30,
                    p,
...
def summary_html_from_json(summary_data):
    thead = (
        "<thead><tr>"
        "<th>DESCRIPTION</th><th>WEIGHT (t)</th><th>LCG (m)</th><th>TCG (m)</th><th>VCG (m)</th>"
        "</tr></thead>"
    )
    ...
    for row in summary_data:
        cells = [
            row.get("name", "-"),
            fmt_num(row.get("total_weight")),
            fmt_num(row.get("total_lcg")),
            fmt_num(row.get("total_tcg")),
            fmt_num(row.get("total_vcg")),
        ]
        row_html = (
            "<tr>"
            f"<td>{cells[0]}</td>"
...
def tank_html_from_json(tank_data):
    new_headers = [
        ("NAME", ""),
        ("FILL", "(%)"),
        ("VOLUME", "(m³)"),
    ...
    thead = "<thead><tr>" + "".join(f"<th>{h[0]}\n{h[1]}</th>" for h in new_headers) + "</tr></thead>"

    # Build rows from tank_data (if you want to display values too)
    rows = []
    for row in tank_data:
        cells = [
            row.get("tank_name", "-"),  
            fmt_num(row.get("fill_percent")),
            fmt_num(row.get("volume")),
...
def cargo_html_from_json(cargo_data) :
    new_headers = [
        ("NAME", ""),
        ("WEIGHT", "(mt)"),
        ("LCG", "(m)"),
...
def strength_html_from_json(strength_data):
    if isinstance(strength_data, str):
        try:
            strength_data = json.loads(strength_data)
        except Exception:
            strength_data = {}

    labels = {
        "sf_limit": "SF<sub>LIMIT</sub>(kN)",
        "sf_max":   "SF<sub>MAX</sub>(kN)",
        "bm_limit": "BM<sub>LIMIT</sub>(kN.m)",
        "bm_max":   "BM<sub>MAX</sub>(kN.m)",
        "uc_sf":    "UC<sub>SF</sub>",
        "uc_bm":    "UC<sub>BM</sub>",
    }
    order = ["sf_limit", "sf_max", "bm_limit", "bm_max", "uc_sf", "uc_bm"]
    decimals_map = {"uc_sf": 2, "uc_bm": 2}
    ...
    def add_rows(d):
        for k in order:
            if k in d:
                dec = decimals_map.get(k, 2)
                label_html = labels.get(k, k)

                value = d.get(k)
                formatted_val = fmt(value, dec)

                # ✅ Conditional coloring for UC values
                if k in ["uc_sf", "uc_bm"]:
                    try:
                        num = float(value)
                        color = "rgb(246, 102, 92)" if num > 1 else "rgb(102, 195, 123)"
                        value_html = f"<td style='color:{color}'>{formatted_val}</td>"
                    except Exception:
                        value_html = f"<td>{formatted_val}</td>"
                else:
                    value_html = f"<td>{formatted_val}</td>"
    ...
    if isinstance(strength_data, dict):
        add_rows(strength_data)
    elif isinstance(strength_data, list):
        for i, d in enumerate(strength_data, start=1):
            rows.append(f"<tr><td colspan='2' style='font-weight:600'>Case {i}</td></tr>")
            if isinstance(d, dict):
                add_rows(d)
    empty_row = '<tr><td colspan="2">No data</td></tr>'
    tbody = f'<tbody>{ "".join(rows) or empty_row }</tbody>'





    thead = "<thead><tr><th>PARAMETER</th><th>VALUE</th></tr></thead>"
    empty_row = '<tr><td colspan="2">No data</td></tr>'
    tbody = f'<tbody>{ "".join(rows) or empty_row }</tbody>'
    html = f"<table>{thead}{tbody}</table>"
    return html

def result_html_from_json(result_data) :
    ...
    labels = {
        "draft_aft" : "T<sub>A</sub>(m)",
        "draft" : "T<sub>M</sub>(m)" ,
        "draft_fwd" : "T<sub>F</sub>(m)",
        "tpc" : "TPC (t/cm)",
        "kmt" : "KM<sub>T</sub>(m)",
        "mct" : "MCT(t.m/cm)",
        "kml" : "KM<sub>l</sub>(m)",
        "gml" : "GM<sub>L</sub>(m)",
        "gmt_liquid" : "GM<sub>T</sub>(m)",
        "lcb" : "LCB (m)",
        "tcb" : "TCB (m)",
        "vcb" : "VCB (m)",
        "lcf" : "LCF (m)",
    ...
    order = ["draft_aft", "draft", "draft_fwd", "tpc", "kmt", "mct", "kml", "gml", "gmt_liquid", "lcb", "tcb", "vcb", "lcf", "heel", "trim"]
    decimals_map = {"draft_aft": 2, "draft": 2, "draft_fwd": 2, "tpc": 2, "kmt": 2, "mct": 2, "kml": 2, "gml": 2, "gmt_liquid": 2, "lcb": 2, "tcb": 2, "vcb": 2, "lcf": 2, "heel": 2, "trim": 2}
    ...
    def fmt(val, decimals=2):
        try:
            ...
            # values are dynamic → keep escaping here
            return escape(str(val)) if val is not None else "-"
    ...
    def add_rows(d):
        for k in order:
            if k in d:
                dec = decimals_map.get(k, 2)
                label_html = labels.get(k, k)   # <-- do NOT escape (contains <sub>)
                rows.append(
                    "<tr>"
                    f"<td>{label_html}</td>"
                    f"<td>{fmt(d.get(k), dec)}</td>"
                    "</tr>"
                )

    if isinstance(result_data, dict):
        add_rows(result_data)
    elif isinstance(result_data, list):
        for i, d in enumerate(result_data, start=1):
            rows.append(f"<tr><td colspan='2' style='font-weight:600'>Case {i}</td></tr>")
            if isinstance(d, dict):
                add_rows(d)

    thead = "<thead><tr><th>PARAMETERS</th><th>VALUES</th></tr></thead>"
    empty_row = '<tr><td colspan="2">No data</td></tr>'
    tbody = f'<tbody>{ "".join(rows) or empty_row }</tbody>'
    html = f"<table>{thead}{tbody}</table>"
    return html