...
def draw_table_with_colors(
    ...
    for index, row in enumerate(soup.find_all("tr")):
        ...
        for cell_index, cell in enumerate(columns):
            ...
            if cell_index == 0:  # Apply subscript formatting only to the first column
                main_text = "".join(
                    BeautifulSoup(
                        cell_content.replace("<sub>", "§").replace("</sub>", "§"),
                        "html.parser",
                ...
                parts = [text]  # No subscript handling for other columns

            cell_color = HexColor("#000000")
            ...
            if index == 0:  # Make header text bold
                canvas.setFont("Helvetica-Bold", 9)
                canvas.setFillColor(header_color)
                canvas.rect(
                    x_position, y_position, cell_width, cell_height, stroke=1, fill=1
...
def generate_ls_plot_graph(table_html):
    ...
    for row in table.find_all("tr")[1:]:  # Skipping the header row
        ...
        if (
            len(cells) >= 5
        ):  # Check if the row has at least five columns (Location, LS, BN, SF, BM)
            try:
                x = float(cells[0].get_text().strip())
                ls = float(cells[1].get_text().strip())
                bn = float(cells[2].get_text().strip())
                sf = float(cells[3].get_text().strip())
                bm = float(cells[4].get_text().strip())
                x_values.append(x)
                ls_values.append(ls)
                bn_values.append(bn)
                sf_values.append(sf)
    ...
    if len(x_values) == 0:
        return None  # Handle the case where there is no data

    # Create the primary plot with a vertical orientation
    fig, ax1 = plt.subplots(figsize=(16, 13))  # Reduced width from 6 to 4
    ...
    # Plotting all the values with separate axes
    (p1,) = ax1.plot(x_values, ls_values, label="WD", linestyle="-", color="blue")
    (p2,) = ax5.plot(x_values, sf_values, label="SF", linestyle="-", color="green")
    (p3,) = ax3.plot(x_values, bn_values, label="BD", linestyle="-", color="red")
    (p4,) = ax4.plot(x_values, bm_values, label="BM", linestyle="-", color="orange")

    ax1.set_xlabel("Location(m)")

    # Set the y-axis label colors and spine colors to match the plot colors
    ax1.yaxis.label.set_color(p1.get_color())
    ax1.spines["left"].set_color(p1.get_color())
    ax5.yaxis.label.set_color(p2.get_color())
    ax5.spines["left"].set_color(p2.get_color())
    ...
    # Determine the maximum absolute value for y-axis limits for each subplot
    max_ls = max(abs(min(ls_values)), abs(max(ls_values)))
    max_bn = max(abs(min(bn_values)), abs(max(bn_values)))
    max_sf = max(abs(min(sf_values)), abs(max(sf_values)))
    max_bm = max(abs(min(bm_values)), abs(max(bm_values)))

    # Add buffer space inside the graph border lines
    buffer_space = 0.1  # Adjust the buffer space as needed
    ax1.set_ylim(-max_ls - buffer_space * max_ls, max_ls + buffer_space * max_ls)
    ax5.set_ylim(-max_sf - buffer_space * max_sf, max_sf + buffer_space * max_sf)
    ax3.set_ylim(-max_bn - buffer_space * max_bn, max_bn + buffer_space * max_bn)
    ax4.set_ylim(-max_bm - buffer_space * max_bm, max_bm + buffer_space * max_bm)

    # Set the x-axis to start from zero
    ax1.set_xlim(0, max(x_values))

    # Set the y-axis tick colors to match the line colors
    ax1.tick_params(axis="y", colors=p1.get_color())
    ax5.tick_params(axis="y", colors=p2.get_color())
    ax3.tick_params(axis="y", colors=p3.get_color())
    ax4.tick_params(axis="y", colors=p4.get_color())

    # Add y-axis grid lines at each x-axis tick for all axes
    ...
    ax5.grid(False, which="both", axis="x")
    ax3.grid(False, which="both", axis="x")
    ax4.grid(False, which="both", axis="x")

    # Remove the default figure border
    for spine in ax1.spines.values():
        spine.set_visible(False)

    # Manually add dashed lines at the top and bottom borders within the layout
    line_top = plt.Line2D(
        [0, 1],
        [1, 1],
    ...
    purple_line = plt.Line2D(
        (0, 1), (0, 0), color="purple", linestyle="--", label="SF_LIMIT"
    )
    blue_line = plt.Line2D(
        (0, 1), (0, 0), color="blue", linestyle="--", label="BM_LIMIT"
    )

    # Add legend above the plot
    fig.legend(
        handles=[p1, p2, p3, p4, purple_line, blue_line],
        loc="upper center",
...
def generate_graph(table_html):
    ...
    for row in table.find_all("tr"):
        cells = row.find_all("td")
        if len(cells) >= 2:  # Check if the row has at least two columns
            try:
                # Convert cell text to float, if fails, skip the row
                x = float(cells[0].get_text().strip())
                y = float(cells[1].get_text().strip())
                x_values.append(x)
                y_values.append(y)
            except ValueError:
                continue  # Skip rows where conversion to float fails
    ...
    # Generate a smoother curve using cubic spline interpolation
    ...
    plt.plot(x_smooth, y_smooth, label="GZ Curve")

    # Grid and labels
    plt.grid(True)
    plt.xlabel("Heel Angle(deg)", fontsize=9)  # X-axis label
    plt.ylabel("Righting Arm(m)", fontsize=9)  # Y-axis label

    # Set y-axis ticks to show increments of 2 instead of 1
    y_min = min(y_values) - 1
    y_max = max(y_values) + 1
    plt.yticks(np.arange(round(y_min, 0), round(y_max, 0) + 2, 2))
...
def check_space_and_add_new_page(
    ...
    global bottom_margin, footer_height
    total_needed_space = needed_space + table_height_estimate
...
def generate_pdf(request):
    ...
    if request.method == "POST":
        ...
        p.drawString(40, title_y_position, "STABILITY ANALYSIS") 
        ...
        p.setFont("Helvetica-Bold", 20)  
        p.setFillColor(HexColor("#000000"))  
        p.drawString(40, title_y_position, "STABILITY ANALYSIS REPORT")
        ...
        sections = [
            ...
            ("     3.2   Results ", report_data),
            ("     3.3   Tank Data ", report_data),
            ("     3.4   Cargo Data ", report_data),
            ("     3.5   GZ Curve ", report_data),
            ("     3.6   GZ Table ", report_data),
            ("     3.7   LS Plot ", report_data),
            ("     3.8   Longitudinal Strength ", report_data),
            ("     3.9   Stability Criteria ", report_data),
            ("     3.10  Weather Criteria ", report_data),
            ("     3.11  Drilling Platform ", report_data),
            ("4   Appendix ", report_data),
            ("     4.1   LS Table ", report_data),
        ]
        ...
        abbreviations = {
            "BM": "Bending moment",
            "BMLIMIT": "Bending moment limit",
            "BMMAX": "Maximum allowable bending moment",
            "FSM": "Free surface moment",
            "GML": "Longitudinal metacentric height",
            "GMT": "Transverse metacentric height",
            "KML": "Longitudinal length of metacenter from keel",
            "KMT": "Transverse height of metacenter from keel",
            "LCB": "Longitudinal center of buoyancy",
            "LCF": "Longitudinal center of floatation",
            "LCG": "Longitudinal center of gravity",
            "MCT": "Moment to change trim by one centimeter",
            "SF": "Shear force",
            "SFLIMIT": "Shear force limit",
            ...
            "UCBM": "Bending moment unity check",
            "UCSF": "Shear force unity check",
        ...
        draw_text(p, "1.2", left_margin, y_position - 20, outside_margin=True)
        draw_text(p, "REFERENCE SYSTEM", left_margin, y_position - 20)
        y_position -= 25
        ...
        # Render the "Perspective Views" section
        draw_text(
            p,
            "2",
            left_margin,
        ...
        soup = BeautifulSoup(request.POST.get("first_table_html"), "html.parser")
        ...
        draw_text(p, "RESULTS", left_margin, y_position - 30)
        y_position -= 10
        caption_text = "Table 3-2 Results"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        soup = BeautifulSoup(request.POST.get("second_table_html"), "html.parser")
        y_position = draw_table_with_subscripts(
            soup,
            y_position - 30,
            p,
        ...
        y_position -= 10
        caption_text = "Table 3-3 Tank Data"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 35
        y_position -= 30
        ...
        new_headers = format_headers(new_headers)

        # Parse the table HTML and modify headers
        soup = BeautifulSoup(request.POST.get("third_table_html"), "html.parser")
        header_row = soup.find("tr")  # Find the first row (header row)
        if header_row:
            for i, th in enumerate(header_row.find_all("th")):
                if i < len(new_headers):
                    th.string = new_headers[i]  # Update header text
        ...
        y_position -= 10
        caption_text = "Table 3-4 Cargo Data"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 35
        y_position -= 30
        ...
        # Parse the table HTML and modify headers
        soup = BeautifulSoup(request.POST.get("fourth_table_html"), "html.parser")
        header_row = soup.find("tr")  # Find the first row (header row)
        if header_row:
            for i, th in enumerate(header_row.find_all("th")):
                if i < len(new_headers):
        ...
        draw_text(p, "GZ CURVE", left_margin, y_position - 30)
        y_position -= 10
        caption_text = "Figure 3-1 GZ Curve"
        p.setFont("Helvetica-Oblique", 10)
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= caption_height
        graph_image_data = generate_graph(request.POST.get("graph_table_html"))
        if graph_image_data:
        ...
        draw_text(p, "GZ TABLE", left_margin, y_position - 30)
        y_position -= 10
        caption_text = "Table 3-5 GZ Table"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        soup = BeautifulSoup(request.POST.get("fifth_table_html"), "html.parser")
        y_position = draw_table(
            soup,
            y_position - 30,
            p,
        ...
        increased_first_column_width = 285
        increased_cell_height = 35
        ...
        draw_text(p, "3.7", left_margin, y_position, outside_margin=True)
        draw_text(p, "LS PLOT", left_margin, y_position)
        caption_text = "Figure 3-2 LS Plot"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 30, caption_text)
        y_position -= 30
        # Extract lsplot_table_html from the form data
        lsplot_table_html = request.POST.get("lsplot_table_html")

        # Generate the LS Plot Graph Image
        graph_image_data = generate_ls_plot_graph(lsplot_table_html)
        if graph_image_data:
            img = Image.open(BytesIO(graph_image_data))

            # Adjust the image size to fit the full page
            desired_width = page_width
            aspect_ratio = img.width / img.height
            desired_height = page_height

            if desired_height > page_height - 2 * margin:
                desired_height = page_height - 3.2 * margin
                desired_width = desired_height * aspect_ratio

            p.drawInlineImage(
                img, margin, margin, width=desired_width, height=desired_height
            )
        ...
        draw_text(p, "3.8", left_margin, y_position, outside_margin=True)
        draw_text(p, "LONGITUDINAL STRENGTH", left_margin, y_position)
        caption_text = "Table 3-6 Longitudinal Strength"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 30, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        soup = BeautifulSoup(request.POST.get("ninth_table_html"), "html.parser")
        y_position = draw_table_with_colors(
            soup,
            y_position - 10,
            p,
            first_column_width=225,
            doc_number=doc_no,
            project_name=project_name,
        )

        estimated_table_height = (
            increased_cell_height * 5
        )  # Estimate based on the increased cell height
        y_position = check_space_and_add_new_page(
            y_position,
            30,
            p,
        ...
        draw_text(p, "STABILITY CRITERIA", left_margin, y_position - 30)
        y_position -= 10
        caption_text = "Table 3-7 Stability Criteria"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        soup = BeautifulSoup(request.POST.get("sixth_table_html"), "html.parser")
        y_position = draw_table_with_colors(
            soup,
            y_position - 30,
            p,
            first_column_width=160,
            second_column_width=65,
            third_column_width=65,
            doc_number=doc_no,
            project_name=project_name,
        )

        estimated_table_height = increased_cell_height * 5
        y_position = check_space_and_add_new_page(
            y_position,
            30,
            p,
        ...
        draw_text(
            p, "3.10", left_margin, y_position - 20, color=black, outside_margin=True
        )
        draw_text(p, "WEATHER CRITERIA", left_margin, y_position - 20, color=black)
        caption_text = "Table 3-8 Weather Criteria"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        soup = BeautifulSoup(request.POST.get("seventh_table_html"), "html.parser")
        y_position = draw_table_with_colors(
            soup,
            y_position - 30,
            p,
            first_column_width=185,
            second_column_width=63,
            third_column_width=60,
            fifth_column_width=35,
            doc_number=doc_no,
            project_name=project_name,
        )
        ...
        center_x = (page_width - text_width) / 2  # Center horizontally

        draw_text(p, text, center_x, y_position - 300, color=HexColor("#2596be"))
        ...
        draw_text(p, "4.1", left_margin, y_position - 30, outside_margin=True)
        draw_text(p, "LS TABLE", left_margin, y_position - 30)
        y_position -= 10
        caption_text = "Table 4-1 LS Table"
        p.setFont("Helvetica-Oblique", 10)  # Set to italic
        p.drawString(left_margin, y_position - 50, caption_text)
        y_position -= 25
        y_position -= 30
        ...
        raw = request.POST.get("eighth_table_html", "")
        if raw:
            try:
                # Prefer JSON → HTML → draw_table to preserve exact styling
                ls_data = json.loads(raw)
                html = build_ls_html_from_json(ls_data)
                soup = BeautifulSoup(html, "html.parser")
                y_position = draw_table(
                    soup,
                    y_position - 30,
                    p,
                    first_column_width=100,   # keep your existing column width config
                    doc_number=doc_no,
                    project_name=project_name,
                )
            except json.JSONDecodeError:
                # Backward compatibility – if the client still sends HTML
...
#     """Format numbers safely for table cells"""
#     try:
#         return f"{float(v):.{places}f}"
#     except Exception:
#         return "-"

# def draw_ls_table_from_json(p, data, y_position, *,
#                             first_col_w=100, doc_number="", project_name=""):
#     """
#     Draws LS Table from JSON like:
#     {
#       "location": [...],
#       "wd": [...],
#       "bd": [...],
...
#     headers = ["LOCATION", "WD (t)", "BD (t)", "SF (kN)", "BM (kNm)"]
#     col_widths = [first_col_w, 90, 90, 110, 110]

#     # Header row
#     y_position = check_space_and_add_new_page(
#         y_position, cell_height, p, doc_number=doc_number, project_name=project_name
#     )
#     draw_table_row(p, headers, y_position, col_widths, header=True)
#     y_position -= cell_height

#     # Safely pull columns from JSON
...
#     row_count = max(len(loc), len(wd), len(bd), len(sf), len(bm))
...
#         )
#         row = [
#             fmt_num(loc[i]) if i < len(loc) else "-",
#             fmt_num(wd[i])  if i < len(wd)  else "-",
#             fmt_num(bd[i])  if i < len(bd)  else "-",
#             fmt_num(sf[i])  if i < len(sf)  else "-",
#             fmt_num(bm[i])  if i < len(bm)  else "-",
#         ]
#         draw_table_row(p, row, y_position, col_widths, header=False)
#         y_position -= cell_height

#     return y_position

# def draw_table_row(p, row_data, y_position, col_widths, header=False):
#     """
#     Draws one row of the LS table.
#     - p: reportlab canvas
#     - row_data: list of cell values (strings/numbers)
#     - y_position: vertical position to draw the row
#     - col_widths: list of column widths
#     - header: True if this is a header row
#     """
#     global cell_height
#     x = 40  # left margin (adjust if you use left_margin variable)
#     font_name = "Helvetica-Bold" if header else "Helvetica"
#     font_size = 10 if header else 9
...
#         width = col_widths[i]
#         # draw cell border
#         p.rect(x, y_position - cell_height, width, cell_height, stroke=1, fill=0)
#         # draw text inside cell
#         p.drawString(x + 2, y_position - cell_height + 5, str(text))
#         x += width
...
def build_ls_html_from_json(ls_data):
    """
    Convert ls_data JSON to an HTML table that matches what draw_table expects.
    ls_data = { "location":[...], "wd":[...], "bd":[...], "sf":[...], "bm":[...] }
    """
    loc = ls_data.get("location", []) or []
    wd  = ls_data.get("wd", []) or []
    bd  = ls_data.get("bd", []) or []
    sf  = ls_data.get("sf", []) or []
    bm  = ls_data.get("bm", []) or []

    n = max(len(loc), len(wd), len(bd), len(sf), len(bm))

    # Header row matches your LS table
    thead = (
        "<thead><tr>"
        "<th>LOCATION</th><th>WD (t)</th><th>BD (t)</th><th>SF (kN)</th><th>BM (kNm)</th>"
        "</tr></thead>"
    )
    ...
    for i in range(n):
        ...
        # Right-align numeric cells via inline style if your draw_table honors it; if not, it’s fine.
        row_html = (
            "<tr>"
            f"<td>{cells[0]}</td>"
            f"<td>{cells[1]}</td>"
